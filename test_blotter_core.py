...
from blotter_core import apply_snapshot_prices, normalize_blotter, parse_treasury_price
...
class BlotterCoreTest(unittest.TestCase):
    def test_parse_treasury_price_handles_plus_and_eighths(self):
        ...
        self.assertAlmostEqual(parse_treasury_price(109.25), 109.25)
    ...
    def test_apply_snapshot_prices_refreshes_matching_quotes(self):
        blotter = [["SR 15Y", -6.0, 4.272], ["TY U6", 25.0, "109-04+"]]
        snapshot = pd.DataFrame(
            [
                {"ticker": "SR 15Y", "last_price": 4.31},
                {"ticker": "TY U6", "last_price": 109.25},
            ]
        )

        refreshed = apply_snapshot_prices(blotter, snapshot)

        self.assertEqual(refreshed, [["SR 15Y", -6.0, 4.31], ["TY U6", 25.0, 109.25]])
